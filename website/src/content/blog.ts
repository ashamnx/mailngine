...
export const posts: BlogPost[] = [
  {
    slug: "introducing-mailngine",
    title: "Introducing Mailngine",
    excerpt:
      "We built Mailngine because developers deserve email infrastructure that just works.",
    date: "2026-04-01",
    readTime: "3 min",
    author: "Mailngine Team",
    content: `
      <p>We built Mailngine because we were tired of fighting with email infrastructure. Every project we worked on had the same story: pick an email provider, wrestle with DNS records, debug deliverability issues, and hope for the best. We knew there had to be a better way.</p>

      <p>Mailngine is the email platform we always wanted. It starts with a clean REST API that feels natural whether you're writing Go, Node.js, or PHP. Domain verification is guided step-by-step, and if you're on Cloudflare, we can configure your DNS records automatically. No more copy-pasting TXT records and wondering if you got the spacing right.</p>

      <p>Under the hood, Mailngine is built on battle-tested open source components. Postfix handles SMTP delivery, OpenDKIM signs every outbound message, and a custom Go service orchestrates everything through a task queue backed by Valkey. Every email gets a delivery receipt, and our webhook system notifies your application in real time with HMAC-signed payloads you can trust.</p>

      <p>We're launching today with support for transactional email, domain management with full DKIM and SPF configuration, and SDKs for Go, Node.js, and Laravel. Analytics, templates, and team management are landing in the coming weeks. We can't wait to see what you build with Mailngine.</p>
    `,
  },
  {
  ...
  },
  {
    ...
    content: `
      ...
      <p>SPF (Sender Policy Framework) tells receiving mail servers which IP addresses are authorized to send email on behalf of your domain. When you add a domain to Mailngine, we provide you with an SPF include record that authorizes our sending infrastructure. You add this as a TXT record on your domain, and receiving servers will check it every time they get an email claiming to be from your domain. Keep in mind that SPF has a 10-lookup limit, so if you use multiple email providers, you'll need to be mindful of how many includes you're chaining together.</p>
